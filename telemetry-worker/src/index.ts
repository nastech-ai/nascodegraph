/**
 * nascodegraph telemetry ingest — telemetry.getnascodegraph.com
 *
 * This file is public on purpose: it is the exact code that receives nascodegraph's
...
const INFO_TEXT = `nascodegraph anonymous-telemetry ingest.

What gets collected (and what never does) is documented field-by-field:
https://github.com/colbymchenry/nascodegraph/blob/main/docs/design/telemetry.md
This endpoint's full source:
https://github.com/colbymchenry/nascodegraph/tree/main/telemetry-worker

Disable any time: nascodegraph telemetry off  |  NASTECHGRAPH_TELEMETRY=0  |  DO_NOT_TRACK=1
`;
...
function sanitizeEvent(raw: unknown, machineId: string, common: JsonObject): PostHogEvent | null {
  ...
  const out: PostHogEvent = {
    ...
    properties: {
      ...
      $geoip_disable: true,
      $lib: 'nascodegraph-telemetry-worker',
    },
  };
