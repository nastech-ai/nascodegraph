...
> **Update (2026-06-01):** the `nascodegraph_trace` and `nascodegraph_context` MCP tools
> were since **removed** — `nascodegraph_explore` is the single surfacing tool now. Its
> "Flow" section (`buildFlowFromNamedSymbols`) and the `nascodegraph_node` trail surface
> these synthesized edges; the `trace(a, b)` notation below means "the a→b flow,"
> which you now verify with `nascodegraph_explore` / `probe-explore.mjs` (the
...
```bash
npm run build
rm -rf /tmp/nascodegraph-corpus/excalidraw/.nascodegraph
( cd /tmp/nascodegraph-corpus/excalidraw && nascodegraph init -i )
# synthesized edges (provenance='heuristic', metadata.synthesizedBy in {callback,event-emitter}):
sqlite3 /tmp/nascodegraph-corpus/excalidraw/.nascodegraph/nascodegraph.db \
...
node scripts/agent-eval/probe-explore.mjs /tmp/nascodegraph-corpus/excalidraw "triggerUpdate triggerRender"
```
...
- **Retrieval/UX changes** (separate from coverage): `explore` whole-small-file + glue
  ...
  — all in `src/mcp/tools.ts`. (`nascodegraph_trace` / `nascodegraph_context` were later
  removed; explore is the one surfacing tool.)
- **Full investigation context + findings:** auto-memory
  `project_nascodegraph_read_displacement` (why coverage — not prompting/hooks/new-tools —
  is the lever for getting agents to use nascodegraph over Read).
