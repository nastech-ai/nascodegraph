 ...
 * NasTech Agent, Gemini CLI, Antigravity IDE).
...
async function runInstallerWithOptions(opts) {
    ...
    if (!useDefaults) {
        ...
        if (shouldInstallGlobally) {
            ...
            try {
                (0, child_process_1.execSync)('npm install -g @nastech-ai/nascodegraph', { stdio: 'pipe', windowsHide: true });
    ...
    if (targets.some((t) => t.id === 'claude')) {
        if (useDefaults) {
            ...
            const ans = await clack.confirm({
                message: 'Front-load NasCodeGraph on “how / where / trace” prompts? Auto-injects structural context so answers need fewer steps (adds a moment to those prompts; Claude Code only).',
                initialValue: true,
            });
            if (clack.isCancel(ans)) {
                clack.cancel('Installation cancelled.');
    ...
    clack.note(location === 'local'
        ...
        : 'cd <your-project>\nnascodegraph init        # build a project’s graph (one time; auto-syncs after)', 'Next: index a project');
...
async function runUninstaller(opts) {
    ...
    if (opts.location) {
        ...
        const sel = await clack.select({
            ...
            options: [
                { value: 'global', label: 'All projects (global)', hint: '~/.claude, ~/.cursor, ~/.codex, ~/.config/opencode, ~/.nastech, ~/.gemini, ~/.kiro' },
    ...
    if (location === 'local' && fs.existsSync((0, directory_1.getNasCodeGraphDir)(process.cwd()))) {
        clack.log.info(`The ${(0, directory_1.codeGraphDirName)()}/ index for this project is still here. Run \`nascodegraph uninit\` to delete it.`);
    }
    ...
    if (removed.length > 0) {
        ...
        clack.outro(`Removed NasCodeGraph from ${removed.length} agent${removed.length > 1 ? 's' : ''}: ${names}. ` +
    ...
    if (opts.yes) {
        ...
        const sel = await clack.select({
            ...
            options: [
                ...
                { value: 'manual', label: 'I\'ll run `nascodegraph sync` myself', hint: 'fully manual' },
            ],
            initialValue: 'hook',
        });
        if (clack.isCancel(sel)) {
