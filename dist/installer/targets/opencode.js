...
function writeMcpEntry(loc) {
    ...
    const edits = (0, jsonc_parser_1.modify)(text, ['mcp', 'nascodegraph'], after, {
        formattingOptions: FORMATTING,
    });
...
function removeMcpEntryAt(file) {
    ...
    const config = parseConfig(text);
    if (!config.mcp?.nascodegraph)
        return { path: file, action: 'not-found' };
    let edits = (0, jsonc_parser_1.modify)(text, ['mcp', 'nascodegraph'], undefined, {
        formattingOptions: FORMATTING,
    });
...
function cleanupLegacyWindowsState() {
    ...
    const action = (0, shared_1.removeMarkedSection)(agents, instructions_template_1.NASTECHGRAPH_SECTION_START, instructions_template_1.NASTECHGRAPH_SECTION_END);
    if (action === 'removed')
