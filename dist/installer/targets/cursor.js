...
function removeRulesEntry() {
    ...
    const startIdx = content.indexOf(instructions_template_1.NASTECHGRAPH_SECTION_START);
    const endIdx = content.indexOf(instructions_template_1.NASTECHGRAPH_SECTION_END);
    // Our marked block is present — strip it, then decide what's left.
    if (startIdx !== -1 && endIdx > startIdx) {
        const before = content.substring(0, startIdx).trimEnd();
        const after = content.substring(endIdx + instructions_template_1.NASTECHGRAPH_SECTION_END.length).trimStart();
