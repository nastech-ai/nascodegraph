...
function logAttachedDaemon(socketPath, hello) {
    if (process.env[LOG_ATTACH_ENV] !== '1')
        return;
    process.stderr.write(`[NasCodeGraph MCP] Attached to shared daemon on ${socketPath} (pid ${hello.pid}, v${hello.nascodegraph}).\n`);
}
/**
 ...
 */
async function runProxy(socketPath, expectedVersion = version_1.NasCodeGraphPackageVersion) {
    ...
    if (hello.nascodegraph !== expectedVersion) {
        process.stderr.write(`[NasCodeGraph MCP] Found a daemon on ${socketPath} but version (${hello.nascodegraph}) ` +
 ...
 */
async function connectWithHello(socketPath, expectedVersion = version_1.NasCodeGraphPackageVersion) {
    if (process.platform !== 'win32' && !fs.existsSync(socketPath))
        return null;
    const socket = net.createConnection(socketPath);
    socket.setEncoding('utf8');
...
async function runLocalHandshakeProxy(deps) {
    ...
    if (socket && !socket.destroyed && !shuttingDown) {
        ...
        const onDaemonLost = () => {
            ...
            daemonSocket = null;
            process.stderr.write(`[NasCodeGraph MCP] Shared daemon connection lost; serving this session in-process (degraded), re-serving ${inflight.size} in-flight request(s).\n`);
