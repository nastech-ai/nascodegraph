 ...
 *
 * Model Context Protocol server that exposes NasCodeGraph functionality
 ...
 * @example
 * ```typescript
 * import { MCPServer } from 'nascodegraph';
 *
 ...
 *   behavior; used when the user opts out (`NASTECHGRAPH_NO_DAEMON=1`), no
 *   `.nascodegraph/` is reachable, or the daemon machinery fails for any reason.
 ...
 *   sharing one NasCodeGraph + watcher + SQLite handle. Spawned on demand; never a
...
exports.NasCodeGraphPackageVersion = exports.Daemon = exports.ToolHandler = exports.tools = exports.StdioTransport = exports.MCPServer = void 0;
 ...
 */
const DAEMON_INTERNAL_ENV = 'NASTECHGRAPH_DAEMON_INTERNAL';
/**
...
// are cheap. Paired with deferring the NasCodeGraph load (engine.ts) off the bind
...
/** Whether `NASTECHGRAPH_NO_DAEMON` was set to a truthy value. */
function daemonOptOutSet() {
    const raw = process.env.NASTECHGRAPH_NO_DAEMON;
    if (!raw)
        return false;
 ...
 * `null` when no `.nascodegraph/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.nascodegraph/`.
 *
...
function resolveDaemonRoot(explicitPath) {
    ...
    const root = (0, directory_1.findNearestNasCodeGraphRoot)(candidate);
    if (!root)
        return null;
    try {
 ...
 * `.nascodegraph/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
...
function spawnDetachedDaemon(root) {
    ...
    let stdio = 'ignore';
    try {
        logFd = fs.openSync(path.join((0, directory_1.getNasCodeGraphDir)(root), 'daemon.log'), 'a');
...
/**
 * MCP Server for NasCodeGraph
 *
 * Implements the Model Context Protocol to expose NasCodeGraph
...
class MCPServer {
     ...
     *
     * Decision order:
     *   1. `NASTECHGRAPH_NO_DAEMON=1` → direct mode (unchanged pre-#411 behavior).
     *   2. `NASTECHGRAPH_DAEMON_INTERNAL=1` → we ARE the detached daemon; listen.
     *   3. No `.nascodegraph/` reachable → direct mode (the daemon's lockfile and
     *      socket both live under `.nascodegraph/`).
    ...
    async start() {
        ...
        if (daemonOptOutSet()) {
            return this.startDirect('NASTECHGRAPH_NO_DAEMON set');
        }
        ...
        if (!root) {
            ...
            return this.startDirect('no .nascodegraph/ root found');
        }
        try {
            ...
            process.stderr.write(`[NasCodeGraph MCP] Proxy path failed (${msg}); falling back to direct mode.\n`);
    ...
    async startDirect(reason) {
        if (reason && process.env.NASTECHGRAPH_MCP_DEBUG) {
            process.stderr.write(`[NasCodeGraph MCP] Direct mode: ${reason}.\n`);
        }
     ...
     * `NASTECHGRAPH_DAEMON_INTERNAL=1`). Arbitrate the O_EXCL lock, then either
    ...
    async startDaemonProcess() {
        ...
        for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
            ...
            if (existing && existing.pid > 0 && (0, daemon_1.isProcessAlive)(existing.pid)) {
                process.stderr.write(`[NasCodeGraph daemon] Another daemon (pid ${existing.pid}) already holds the lock; exiting.\n`);
                process.exit(0);
            }
        ...
        process.stderr.write('[NasCodeGraph daemon] Could not acquire the daemon lock; exiting.\n');
        process.exit(0);
    }
    /**
    ...
    installPpidWatchdog() {
        if (this.mode !== 'direct')
            return;
        const pollMs = (0, ppid_watchdog_1.parsePpidPollMs)(process.env.NASTECHGRAPH_PPID_POLL_MS);
        if (pollMs <= 0)
            return;
...
Object.defineProperty(exports, "NasCodeGraphPackageVersion", { enumerable: true, get: function () { return version_1.NasCodeGraphPackageVersion; } });
