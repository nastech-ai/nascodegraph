...
class Daemon {
    ...
    async start() {
        ...
        try {
            bound = await bindFirstUsableSocket(candidates, listen, {
                onRelocate: (from, to, code) => process.stderr.write(`[NasCodeGraph daemon] Socket ${from} unusable (${code}); relocating to ${to}.\n`),
            });
        }
        catch (err) {
        ...
        const lock = {
            pid: process.pid,
            version: version_1.NasCodeGraphPackageVersion,
        ...
        process.stderr.write(`[NasCodeGraph daemon] Listening on ${this.socketPath} (pid ${process.pid}, v${version_1.NasCodeGraphPackageVersion}). Idle timeout ${this.idleTimeoutMs}ms.\n`);
    ...
    handleConnection(socket) {
        ...
        const hello = {
            nascodegraph: version_1.NasCodeGraphPackageVersion,
            pid: process.pid,
            socketPath: this.socketPath,
            protocol: 1,
        };
    ...
    reapDeadClients(isAlive) {
        ...
        for (const session of [...this.clients]) {
            ...
            if (!peers || !peerIsDead(peers, isAlive))
                continue;
            process.stderr.write(`[NasCodeGraph daemon] Reaping client with dead peer (pid ${peers.pid}); clients=${this.clients.size - 1}.\n`);
            try {
                session.stop();
            }
            catch { /* best-effort */ }
...
function parseClientHelloLine(line) {
    ...
    if (!parsed || typeof parsed !== 'object')
        return null;
    const o = parsed;
    if (o.nascodegraph_client !== 1 || typeof o.pid !== 'number')
        return null;
