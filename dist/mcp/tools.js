...
function formatDegradedBanner(reason) {
    return ('⚠️ NasCodeGraph auto-sync is DISABLED — live file watching stopped, so the index is ' +
...
function getStaticTools() {
    ...
    if (!raw || !raw.trim()) {
        return exports.tools.filter(t => DEFAULT_MCP_TOOLS.has(t.name.replace(/^nascodegraph_/, '')));
    }
    const allow = new Set(raw.split(',').map(s => s.trim().replace(/^nascodegraph_/, '')).filter(Boolean));
    return allow.size ? exports.tools.filter(t => allow.has(t.name.replace(/^nascodegraph_/, ''))) : exports.tools;
}
/**
...
class ToolHandler {
    ...
    async awaitCatchUpGate(gate) {
        ...
        try {
            ...
            if (outcome === 'timeout') {
                process.stderr.write(`[NasCodeGraph MCP] Catch-up reconcile still running after ${timeoutMs}ms; serving this tool call now and finishing the reconcile in the background (#905). ` +
    ...
    toolAllowlist() {
        ...
        if (!raw || !raw.trim())
            return null;
        const short = (s) => s.trim().replace(/^nascodegraph_/, '');
    ...
    getTools() {
        ...
        let visible = allow
            ? exports.tools.filter(t => allow.has(t.name.replace(/^nascodegraph_/, '')))
            : exports.tools.filter(t => DEFAULT_MCP_TOOLS.has(t.name.replace(/^nascodegraph_/, '')));
     ...
     */
    getNasCodeGraph(projectPath) {
        if (!projectPath) {
            if (!this.cg) {
                const searched = this.defaultProjectHint ?? process.cwd();
                throw new NotIndexedError('No NasCodeGraph project is loaded for this session.\n' +
        ...
        const resolvedRoot = (0, directory_1.findNearestNasCodeGraphRoot)(projectPath);
        if (!resolvedRoot) {
            throw new NotIndexedError(`The project at ${projectPath} isn't indexed with nascodegraph (no .nascodegraph/ directory found ` +
    ...
    freshen(cg) {
        try {
            if (cg.reopenIfReplaced()) {
                process.stderr.write('[NasCodeGraph MCP] The index was replaced on disk (e.g. a git worktree ' +
    ...
    async handleSearch(args) {
        ...
        if (typeof query !== 'string')
            return query;
        const cg = this.getNasCodeGraph(args.projectPath);
    ...
    async handleNode(args) {
        ...
        if (listed.length) {
            ...
            out.push('', `> Need one of these in full? Call nascodegraph_node again with \`file\` (e.g. \`"${listed[0].filePath.split('/').pop()}"\`) or \`line\` — do NOT Read it.`);
        }
    ...
    async handleFileView(cg, fileArg, opts = {}) {
        ...
        if (!complete) {
            out.push('', `(lines ${offset}–${shownEnd} of ${total} — pass \`offset\`/\`limit\` for another range, or \`nascodegraph_node <symbol>\` for one symbol in full)`);
        }
    ...
    formatTrail(cg, node) {
        ...
        if (callees.length === 0 && callers.length === 0)
            return '';
        const lines = ['', '**Trail — nascodegraph_node any of these to follow it (no Read needed)**'];
        if (callees.length > 0) {
    ...
    async handleStatus(args) {
        let cg = this.getNasCodeGraph(args.projectPath);
    ...
    formatNodeDetails(node, code, outline) {
        ...
        if (outline) {
            lines.push('', outline, '', `> Structural outline only. Read \`${node.filePath}\` or call nascodegraph_node on a specific member for its body.`);
        }
        else if (code) {
