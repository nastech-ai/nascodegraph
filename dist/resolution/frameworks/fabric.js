...
exports.fabricViewResolver = void 0;
const CODEGEN_DECL_RE = /codegenNativeComponent\s*(?:<[^>]+>)?\s*\(\s*['"]([A-Za-z_][A-Za-z0-9_]*)['"]/g;
/**
 ...
 */
const RCT_VIEW_PROP_RE = /\bRCT_(?:EXPORT|CUSTOM|REMAP)_VIEW_PROPERTY\s*\(\s*([A-Za-z_][A-Za-z0-9_]*)/g;
/**
 ...
 */
function deriveComponentNameFromManager(className) {
    let name = className.startsWith('RCT') ? className.slice(3) : className;
    // Trim ViewManager > Manager > View, in order.
    if (name.endsWith('ViewManager'))
        name = name.slice(0, -'ViewManager'.length);
    else if (name.endsWith('Manager'))
        name = name.slice(0, -'Manager'.length);
    return name;
}
/**
 ...
 */
function findNativePropsBody(source) {
 ...
 */
function extractLegacyViewManagerNodes(filePath, source) {
    ...
    if (!className.endsWith('Manager') && !className.endsWith('ViewManager'))
        return [];
 ...
 */
function extractJvmViewManagerNodes(filePath, source) {
    if (!source.includes('@ReactProp'))
        return [];
...
function extractFabricNodes(filePath, source) {
    ...
    if (body) {
        ...
        for (const propName of props) {
            ...
            const propLine = propBefore >= 0 ? source.slice(0, propBefore).split('\n').length : 1;
            nodes.push({
                id: `fabric-prop:${filePath}:${propName}:${propLine}`,
                kind: 'property',
                name: propName,
    ...
    detect(context) {
        ...
        for (const root of ['packages', 'apps', 'modules', 'libraries']) {
            for (const sub of list(root) ?? []) {
                if (checkPkg(`${root}/${sub}/package.json`))
                    return true;
            }
        }
        return false;
