...
function stripAgentDirectives(context) {
    ...
    while (i < lines.length) {
        ...
        if (/^>\s/.test(ln) && /(do NOT re-read|Complete source for|Explore budget:|file sections were trimmed|nascodegraph_explore|complete than (reading|Read)|Reserve Read|falling back to Read|Synthesize once)/i.test(ln)) {
            i++;
            continue;
        }
