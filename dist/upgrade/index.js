...
exports.REPO = 'nastech-ai/nascodegraph';
exports.NPM_PACKAGE = '@nastech-ai/nascodegraph';
...
async function resolveLatestVersion(repo = exports.REPO, timeoutMs = 12000) {
    try {
        const res = await httpsGet(`https://github.com/${repo}/releases/latest`, { 'User-Agent': 'nascodegraph-upgrade' }, timeoutMs);
    ...
    try {
        const res = await httpsGet(`https://api.github.com/repos/${repo}/releases/latest`, { 'User-Agent': 'nascodegraph-upgrade', Accept: 'application/vnd.github+json' }, timeoutMs);
    ...
    throw new Error('could not resolve the latest version from GitHub. Check your network, or pin a version: `nascodegraph upgrade <version>`.');
}
const c = {
...
async function selfHealPromptHook(deps) {
    ...
    if (res.action === 'created' || res.action === 'updated') {
        deps.log(c.dim('Enabled the NasCodeGraph front-load hook for Claude Code (structural prompts). Disable any time: NASTECHGRAPH_NO_PROMPT_HOOK=1'));
    }
}
function upgradeUnixBundle(method, pinned, deps) {
    ...
    if (method.installDir)
        env.NASTECHGRAPH_INSTALL_DIR = method.installDir;
    if (pinned)
        env.NASTECHGRAPH_VERSION = pinned;
...
function buildWindowsUpgradeScript(bundleRoot, version, arch) {
    ...
    const url = `https://github.com/${exports.REPO}/releases/download/${version}/nascodegraph-${target}.zip`;
