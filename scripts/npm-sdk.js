'use strict';
//
// Programmatic / embedded SDK entry for @nastech-ai/nasnascodegraph (issue #354).
//
...
// API — `require("@nastech-ai/nasnascodegraph")` returning the NasCodeGraph class et al.
//
// The compiled library + its production dependencies (web-tree-sitter,
// tree-sitter-wasms, …) ship INSIDE the per-platform bundle, at
//   @nastech-ai/nasnascodegraph-<platform>-<arch>/lib/dist/index.js
...
function resolveLibrary() {
  ...
  throw new Error(
    'nascodegraph: the programmatic API is unavailable because the platform bundle\n' +
    ...
    '      npm i @nastech-ai/nasnascodegraph --registry=https://registry.npmjs.org\n' +
    '  - or run the CLI once (e.g. `npx @nastech-ai/nasnascodegraph status`) to\n' +
    '    self-heal the bundle into ~/.nascodegraph, then require() will find it.'
  );
}
...
function cachedLibrary() {
  try {
    ...
    var base = process.env.NASTECHGRAPH_INSTALL_DIR || path.join(os.homedir(), '.nascodegraph');
