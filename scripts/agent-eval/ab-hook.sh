#!/usr/bin/env bash
# A/B the PreToolUse(Read) REDIRECT hook (P1): does steering Read → nascodegraph_node
# file-view actually move the agent off Read during implementation? BOTH arms use
# the CURRENT build with nascodegraph attached and pre-warmed; the only difference is
...
#
#   arm [nohook] — nascodegraph on, no hook   (does the better file-view get picked on its own?)
#   arm [hook]   — nascodegraph on, + redirect hook   (does routing close it?)
#
# Reliable attach (works nested): each arm pre-warms a persistent daemon and skips
# the startup re-exec (NASTECHGRAPH_WASM_RELAUNCHED=1), so claude connects before the
# agent's first turn. Judge by ACTUAL nascodegraph usage in parse-run.mjs's "by type",
...
#   <indexed-repo>  a repo with a .nascodegraph index (copied per arm; never mutated)
...
echo "###### mcp__nascodegraph__nascodegraph_node should rise in the [hook] arm. Logs: $OUT"
