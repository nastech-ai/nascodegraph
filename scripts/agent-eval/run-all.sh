#!/usr/bin/env bash
# With/without A/B (and optional interactive) eval for a nascodegraph version on a
# repo. Codegraph is the ONLY variable: both arms launch claude with
# --strict-mcp-config — with = nascodegraph-only MCP (pointed at $CG_BIN),
...
# Env:   CG_BIN          nascodegraph binary (default: command -v nascodegraph)
...
[ -n "$CG_BIN" ] || { echo "no nascodegraph binary on PATH (set CG_BIN)"; exit 1; }
[ -d "$REPO/.nascodegraph" ] || { echo "no .nascodegraph index at $REPO — index it first"; exit 1; }
...
if [ "$MODE" = headless ] || [ "$MODE" = all ]; then
  headless "headless-with"    "$OUT/mcp-nascodegraph.json"
  headless "headless-without" "$OUT/mcp-empty.json"
fi

if [ "$MODE" = tmux ] || [ "$MODE" = all ]; then
  echo "############################## INTERACTIVE [with] ##############################"
  CLAUDE_EXTRA_ARGS="--model ${MODEL:-sonnet} --effort ${EFFORT:-high} --strict-mcp-config --mcp-config $OUT/mcp-nascodegraph.json" \
