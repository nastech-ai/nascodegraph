#!/usr/bin/env bash
# FRONTLOAD arm (approach 1): nascodegraph attached (offload-disabled) + the front-load
...
printf '{"mcpServers":{"nascodegraph":{"command":"env","args":["NASTECHGRAPH_WASM_RELAUNCHED=1","NASTECHGRAPH_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TARGET" > "$CFG"
...
prewarm() {
  ...
  env NASTECHGRAPH_OFFLOAD_DISABLE=1 NASTECHGRAPH_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$1" </dev/null >/dev/null 2>&1 &
