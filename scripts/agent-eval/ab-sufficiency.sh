...
# nascodegraph (explore/node), does it still Read? Premise under test: explore/node
# return source WITH line numbers, so a Read should not be needed.
#
# WITH nascodegraph (pre-warmed daemon, reliable nested attach) vs WITHOUT (empty
...
# (configs, docs, a file nascodegraph didn't index).
#
...
printf '{"mcpServers":{"nascodegraph":{"command":"env","args":["NASTECHGRAPH_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TGT" > "$OUT/mcp-cg.json"

prewarm(){
  pkill -9 -f "serve --mcp --path $TGT" 2>/dev/null
  NASTECHGRAPH_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$TGT" </dev/null >/dev/null 2>&1 &
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.nascodegraph/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$TGT" >/dev/null 2>&1
}
...
echo "== WITH nascodegraph (premise: explore/node used -> Read ~0) =="; run with "$OUT/mcp-cg.json" 1
