...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; ENGINE="$(cd "$HERE/../.." && pwd)"; BIN="$ENGINE/dist/bin/nascodegraph.js"
...
# `raw` is a pseudo-style: nascodegraph attached but the offload DISABLED (the ceiling —
# verbatim source, no reasoning model). Any other value is an offload style (plain|refs).
if [ "$STYLE" = "raw" ]; then
  DAEMON_ENV="NASTECHGRAPH_OFFLOAD_DISABLE=1"
  printf '{"mcpServers":{"nascodegraph":{"command":"env","args":["NASTECHGRAPH_WASM_RELAUNCHED=1","NASTECHGRAPH_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' \
    "$BIN" "$TARGET" > "$CFG"
  USAGE="-"
else
  DAEMON_ENV="NASTECHGRAPH_OFFLOAD_STYLE=$STYLE NASTECHGRAPH_OFFLOAD_USAGE_LOG=$USAGE"
  printf '{"mcpServers":{"nascodegraph":{"command":"env","args":["NASTECHGRAPH_WASM_RELAUNCHED=1","NASTECHGRAPH_OFFLOAD_STYLE=%s","NASTECHGRAPH_OFFLOAD_USAGE_LOG=%s","node","%s","serve","--mcp","--path","%s"]}}}' \
    "$STYLE" "$USAGE" "$BIN" "$TARGET" > "$CFG"
fi

# Prewarm a persistent daemon carrying the SAME offload config (it does the reasoning).
pkill -9 -f "serve --mcp --path $TARGET" 2>/dev/null; rm -f "$TARGET/.nascodegraph/daemon.sock" 2>/dev/null; sleep 0.6
env $DAEMON_ENV NASTECHGRAPH_DAEMON_IDLE_TIMEOUT_MS=1800000 \
  node "$BIN" serve --mcp --path "$TARGET" </dev/null >/dev/null 2>&1 &
node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.nascodegraph/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$TARGET" \
...
# salience, ignores nascodegraph, and thrashes via Bash+Read, making the A/B noise).
...
pkill -9 -f "serve --mcp --path $TARGET" 2>/dev/null; rm -f "$TARGET/.nascodegraph/daemon.sock" 2>/dev/null
