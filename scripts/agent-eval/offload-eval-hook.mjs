...
// Front-loads nascodegraph's structural answer for flow/impact/"how/where" prompts so the
...
for (let i = 0; i < 6 && root; i++) {
  if (existsSync(join(root, '.nascodegraph'))) { found = root; break; }
  const parent = resolve(root, '..'); if (parent === root) break; root = parent;
}
if (!found) { dbg(`no .nascodegraph found from cwd=${cwd}`); process.exit(0); }
dbg(`found index at ${found}`);

try {
  process.env.NASTECHGRAPH_OFFLOAD_DISABLE = '1'; // raw, accurate — never the unfixed offload
  process.env.NASTECHGRAPH_TELEMETRY = '0'; process.env.DO_NOT_TRACK = '1';
  ...
  if (typeof ___NASNASTECHGRAPH___?.openSync !== 'function' || typeof ToolHandler !== 'function') process.exit(0);
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const cg = ___NASNASTECHGRAPH___.openSync(found);
      const h = new ToolHandler(cg);
      const res = await h.execute('nascodegraph_explore', { query: prompt });
  ...
  process.stdout.write(
    `## ___NASNASTECHGRAPH___ structural context (auto-retrieved for this question)\n` +
    `The code graph was queried for your question; the relevant symbols, source, and call flow are below. ` +
    `Treat the quoted source as already read. If you need more, call nascodegraph_explore with specific symbol names rather than grepping or reading files.\n\n` +
    text + '\n'
  );
