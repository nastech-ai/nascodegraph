...
#   ARM offload : nascodegraph attached, managed offload ON  (per-run AI usage log)
#   ARM raw     : nascodegraph attached, NASTECHGRAPH_OFFLOAD_DISABLE=1 (raw source)
#   ARM nocg    : no nascodegraph (empty MCP config) -> Read/Grep baseline
...
[ -d "$TARGET/.nascodegraph" ] || { echo "not indexed: $TARGET (run offload-eval-setup.sh first)"; exit 1; }
...
run() { # arm rep mcp-config usage-log-or-dash
  ...
  # measures the retrieval mode, not whether Sonnet decides to spawn a nascodegraph-blind
...
printf '{"mcpServers":{"nascodegraph":{"command":"env","args":["NASTECHGRAPH_WASM_RELAUNCHED=1","NASTECHGRAPH_OFFLOAD_USAGE_LOG=%s","node","%s","serve","--mcp","--path","%s"]}}}' "$USAGE" "$BIN" "$TARGET" > "$CFG_OFF"
printf '{"mcpServers":{"nascodegraph":{"command":"env","args":["NASTECHGRAPH_WASM_RELAUNCHED=1","NASTECHGRAPH_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TARGET" > "$CFG_RAW"
...
echo "###### Q=$Q"
echo "== ARM offload =="; prewarm "$TARGET" "NASTECHGRAPH_OFFLOAD_USAGE_LOG=$USAGE"
for r in $(seq "$START" "$END"); do run offload "$r" "$CFG_OFF" "$USAGE"; done
pkill -9 -f "serve --mcp --path $TARGET" 2>/dev/null; rm -f "$TARGET/.nascodegraph/daemon.sock" 2>/dev/null; sleep 1
