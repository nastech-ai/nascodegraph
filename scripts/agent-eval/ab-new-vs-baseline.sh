#!/usr/bin/env bash
# A/B a nascodegraph retrieval/steering change: the NEW build (current HEAD) vs a
# BASELINE build (a git ref) — BOTH with nascodegraph attached — on the same
# implementation task, measuring how many Read vs nascodegraph calls the agent
...
# session): each arm PRE-WARMS a persistent nascodegraph daemon for its target so
# claude connects to an already-bound, index-loaded daemon instantly — before
# the agent's first turn — and SKIPS nascodegraph's startup re-exec via
# NASTECHGRAPH_WASM_RELAUNCHED=1. Without this, on a multi-step task the agent
# dives into Read/grep before nascodegraph finishes its ~2-3s startup (worse under
# the CPU contention of a nested run) and runs with NO nascodegraph.
#
# Gotcha: claude's `system/init` snapshot can read status:"pending" / 0 tools
# even when the server then connects fine — judge by ACTUAL nascodegraph usage in
# parse-run.mjs's "by type", not the init line.
#
# Usage: ab-new-vs-baseline.sh <indexed-repo> "<task>" [baseline-ref]
#   <indexed-repo>  a repo with a .nascodegraph index (copied per arm)
...
rsync -a --exclude node_modules --exclude .git --exclude dist --exclude .nascodegraph "$TARGET/" "$OUT/t-new/"
...
prewarm() { # target — spawn a persistent daemon (current $BIN) and wait for its socket
  ...
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.nascodegraph/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$1" \
    && echo "  daemon warm: $1" || echo "  WARN: daemon never bound for $1 (arm may run without nascodegraph)"
}
...
echo "###### (especially Read vs mcp__nascodegraph__*). Full logs in: $OUT"
