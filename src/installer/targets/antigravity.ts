 ...
 * was then auto-migrated by Antigravity, and re-ran `nascodegraph install`
 * doesn't end up with stale nascodegraph entries in two files.
 *
 ...
 * to get nascodegraph to appear in the Customizations UI. We build the
 ...
 * tools live outside that, so a bare `nascodegraph` command fails to spawn
 * even when `which nascodegraph` resolves in the user's shell. We resolve
 * `nascodegraph` to its absolute path on macOS at install time. (Linux GUI
 ...
 * still get a working MCP integration; the prefer-nascodegraph-over-grep
...
/**
 * Resolve the on-disk path of the `nascodegraph` binary so a Mac GUI app
 * launched from Dock/Finder (with a stripped PATH) can find it. Falls
 * back to the bare `nascodegraph` name when:
 *
...
function resolveCodegraphCommand(): string {
  if (process.platform !== 'darwin') return 'nascodegraph';
  try {
    const resolved = execSync('command -v nascodegraph || which nascodegraph', {
      encoding: 'utf-8',
...
/**
 * Build the nascodegraph MCP-server entry for Antigravity. Distinct from
...
class AntigravityTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // wind up with two competing nascodegraph configs.
  ...
  uninstall(loc: Location): WriteResult {
    ...
    // vice versa) — handles the migration-half-state case where nascodegraph
  ...
  printConfig(loc: Location): string {
    ...
    const file = preferredMcpConfigPath();
    const snippet = JSON.stringify({ mcpServers: { nascodegraph: buildAntigravityEntry() } }, null, 2);
    return `# Add to ${file}\n\n${snippet}\n`;
  }
...
/**
 * Strip the nascodegraph entry from the legacy `~/.gemini/antigravity/mcp_config.json`
 * if it's present AND we're writing to the unified path. Used by install
 * to migrate users who had nascodegraph configured on the legacy path
...
function cleanupLegacyEntry(): WriteResult['files'][number] | null {
  ...
  if (!config.mcpServers?.nascodegraph) return null;
...
function removeCodegraphFromFile(file: string): WriteResult['files'][number] {
  ...
  const config = readJsonFile(file);
  if (!config.mcpServers?.nascodegraph) return { path: file, action: 'not-found' };
  delete config.mcpServers.nascodegraph;
  if (Object.keys(config.mcpServers).length === 0) {
    delete config.mcpServers;
  }
