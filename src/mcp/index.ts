...
import { findNearestNasCodeGraphRoot, getNasCodeGraphDir } from '../directory';
...
function resolveDaemonRoot(explicitPath: string | null): string | null {
  ...
  const root = findNearestNasCodeGraphRoot(candidate);
  if (!root) return null;
...
function spawnDetachedDaemon(root: string): void {
  ...
  try {
    logFd = fs.openSync(path.join(getNasCodeGraphDir(root), 'daemon.log'), 'a');
...
export class MCPServer {
    ...
    for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
      ...
      if (existing && existing.pid > 0 && isProcessAlive(existing.pid)) {
        process.stderr.write(
          `[NasCodeGraph daemon] Another daemon (pid ${existing.pid}) already holds the lock; exiting.\n`
        );
        process.exit(0);
      }
  ...
  private installPpidWatchdog(): void {
    ...
    const pollMs = parsePpidPollMs(process.env.NASTECHGRAPH_PPID_POLL_MS);
    if (pollMs <= 0) return;
    this.ppidWatchdog = setInterval(() => {
      ...
      if (reason) {
        process.stderr.write(
          `[NasCodeGraph MCP] Parent process exited (${reason}); shutting down.\n`
        );
        this.stop();
      }
    }, pollMs);
...
export { NasCodeGraphPackageVersion } from './version';
