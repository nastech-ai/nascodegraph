 ...
 *  1. `NASTECHGRAPH_NO_WATCH=1`    → off  (explicit opt-out always wins)
 *  2. `NASTECHGRAPH_FORCE_WATCH=1` → on   (overrides auto-detection)
...
export function watchDisabledReason(projectRoot: string, probe: WatchProbe = {}): string | null {
  ...
  if (env.NASTECHGRAPH_NO_WATCH === '1') {
    return 'NASTECHGRAPH_NO_WATCH=1 is set';
  }
  if (env.NASTECHGRAPH_FORCE_WATCH === '1') {
    return null;
  }
