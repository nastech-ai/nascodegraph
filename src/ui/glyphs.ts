 ...
 *   - `NASTECHGRAPH_ASCII=1`  -> ASCII (escape hatch for any terminal)
 *   - `NASTECHGRAPH_UNICODE=1` -> Unicode (opt-in on Windows)
...
export function supportsUnicode(): boolean {
  if (process.env.NASTECHGRAPH_ASCII === '1') return false;
  if (process.env.NASTECHGRAPH_UNICODE === '1') return true;
