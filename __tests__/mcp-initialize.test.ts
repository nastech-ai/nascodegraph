 ...
 * the MCP server was blocking the initialize response on NasCodeGraph.open() and
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // daemon (logging to .nascodegraph/daemon.log, not the child's stderr); the
...
describe('MCP initialize handshake (issue #172)', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nascodegraph-mcp-init-'));
  });
  ...
  it('responds to initialize quickly when no .nascodegraph exists in cwd', async () => {
  ...
  it('sends initialize response BEFORE tryInitializeDefault finishes', async () => {
    // Seed a real .nascodegraph so the server's tryInitializeDefault path runs
    // its full body: NasCodeGraph.open() (which awaits initGrammars()) and then
    ...
    const json = JSON.parse(response.text);
    expect(json.id).toBe(0);
    expect(json.result.serverInfo.name).toBe('nascodegraph');
  }, 20000);
