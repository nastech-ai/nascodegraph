...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nascodegraph-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use NasCodeGraph integration)
// =============================================================================
...
`);
    ...
    const cg = NasCodeGraph.initSync(testDir, {
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'nascodegraph.db');
...
`);
      ...
      // note) so nascodegraph_node can hand back every overload and the agent never
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via NasCodeGraph.uninitialize()', async () => {
    const NasCodeGraph = (await import('../src/index')).default;

    // Initialize
    const cg = NasCodeGraph.initSync(testDir);
    expect(NasCodeGraph.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .nascodegraph directory should be removed
    expect(NasCodeGraph.isInitialized(testDir)).toBe(false);
  });
});
