 ...
 * started outside any repo, or a monorepo root whose `.nascodegraph/` indexes live
...
const exploreOf = (defs: { name: string; inputSchema: { required?: string[] } }[]) =>
  defs.find((t) => t.name === 'nascodegraph_explore')!;
...
describe('No-default-project requires projectPath in the schema (#993)', () => {
  ...
  it('marks projectPath required on nascodegraph_explore when no default project is loaded', () => {
  ...
  it('a missing projectPath with no default is still SUCCESS-shaped guidance, not isError', async () => {
    ...
    const res = await new ToolHandler(null).execute('nascodegraph_explore', { query: 'anything' });
...
describe('A default project keeps projectPath OPTIONAL (#993)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nascodegraph-reqpath-'));
    fs.writeFileSync(
  ...
  it('a bare call (no projectPath) still falls back to the default project', async () => {
    const res = await new ToolHandler(cg).execute('nascodegraph_explore', { query: 'processPayment' });
    expect(res.isError).toBeUndefined();
    // Resolved against the default project — not the no-default guidance.
    expect(res.content[0]!.text).not.toMatch(/No NasCodeGraph project is loaded/);
