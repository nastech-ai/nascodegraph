...
NasCodeGraph collects a small set of **anonymous usage statistics** — which commands and
...
```bash
nascodegraph telemetry off        # stores your choice (and deletes any unsent data)
```

```bash
export NASTECHGRAPH_TELEMETRY=0   # per-shell / per-CI override
export DO_NOT_TRACK=1          # the cross-tool standard — always honored
```

`nascodegraph telemetry status` shows the current state, what decided it, and your machine ID.
The interactive installer (`nascodegraph install`) asks up front with a visible default-on
...
Off means off: when disabled, NasCodeGraph records nothing, opens no connection to the
...
- **`install`** — when `nascodegraph install` configures agents: which agents
...
- **`usage_rollup`** — one line per day per tool: the tool or CLI command **name** (e.g.
  `nascodegraph_explore`, `init`), how many times it ran, how many errored, and — for MCP
  tools — the connecting agent's name and version from the MCP handshake (e.g.
  `Claude Code 2.1`).
- **`uninstall`** — when `nascodegraph uninstall`/`uninit` runs: which agents were removed.
...
- **No fingerprinting.** The machine ID is a random UUID stored in
  `~/.nascodegraph/telemetry.json` — delete that file (or run `nascodegraph telemetry off`,
...
Events POST to `telemetry.getnascodegraph.com` — a first-party endpoint whose complete
