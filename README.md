...
<a href="https://getcodegraph.com"><img alt="Join the waitlist for early beta access" src="https://raw.githubusercontent.com/nastech-ai/nascodegraph/main/assets/waitlist.svg?v=2" height="52"></a>
...
<sub>Detects and auto-configures Claude Code, Cursor, Codex CLI, opencode, NasTech Agent, Gemini CLI, Antigravity IDE, and Kiro — wiring the CodeGraph MCP server into each. **This is the step that connects CodeGraph to your agent;** installing the CLI in step 1 does not do it on its own. It only wires up your agent — it does **not** index any code; building each project's graph is the separate `codegraph init` in step 3. (Shortcut: `npx @nastech-ai/nascodegraph` downloads and runs this in one go.)</sub>
...
```typescript
import CodeGraph from '@nastech-ai/nascodegraph';
// CommonJS works too:
//   const { CodeGraph } = require('@nastech-ai/nascodegraph');
